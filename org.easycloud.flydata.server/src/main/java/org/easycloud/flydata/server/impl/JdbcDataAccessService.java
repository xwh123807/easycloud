...
import org.easycloud.flydata.server.repository.DataSourceRepository;
...
public class JdbcDataAccessService implements IJdbcDataAccessService {
	...
	private JdbcTemplate getJdbcTemplate(String dataSourceId) {
		if (StringUtils.isBlank(dataSourceId)) {
			...
			try {
				return new JdbcTemplate(dataSourceRepository.getDataSource(dataSourceId));
			} catch (Exception e) {
	 ...
	 */
	@Override
	public List<Map<String, Object>> findAll(String dataSourceId, String tableName, String where,
			int page, int size, Sort sort) {
		AssertUtil.parameterEmpty(tableName, "tableName");
